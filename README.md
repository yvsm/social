...
基于 https://github.com/liliuwei/thinkphp-social 修改


## 安装（扩展包）
```php
composer require zunyunkeji/social
```
...
class Oauth extends Controller
{
    //登录地址
        public function login($type = null)
        {
            ...
            // 获取对象实例
            $sns = \zunyunkeji\social\Oauth::getInstance($type);
            //跳转到授权页面
        ...
        public function callback($type = null, $code = null)
        {
            ...
            //获取当前第三方登录用户信息
            if (is_array($token)) {
                $user_info = \zunyunkeji\social\GetInfo::getInstance($type, $token);
